...
# 0) Turn LeakSanitizer off at BUILD time: compile the fleet's __lsan_is_turned_off() hook
#    (mayhem/lsan_off.cc) with the SAME sanitizer flags as the target and hand the object to
#    CMake's exe link step below, so it is linked into the `cs` fuzz binary. `cs` is an
#    allocate-and-exit interpreter — it std::exit()s without tearing down its process pools,
#    contexts or per-unit token arenas — so leak reporting would fire on nearly every input and
#    bury the real bugs. ASan bounds/UAF and UBSan stay fully ON and halting; only leak
#    reporting goes away. Guarded on ASan actually being active, so an explicit empty
#    SANITIZER_FLAGS build (--build-arg SANITIZER_FLAGS=) stays clean.
EXE_LINK_FLAGS=""
if printf '%s' "$SANITIZER_FLAGS" | grep -q address; then
	"$CXX" $SANITIZER_FLAGS $DEBUG_FLAGS -c "$SRC/mayhem/lsan_off.cc" -o /tmp/lsan_off.o
	EXE_LINK_FLAGS="/tmp/lsan_off.o"
fi
